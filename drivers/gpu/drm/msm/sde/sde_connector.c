		...
		bool en)
{
	...
	if (!c_conn)
       ...
       if(((lcd_states != DISPLAY_PRIMARY)||(lcd_states != DISPLAY_ONLY_PRIMARY))&&(en) ){
            pr_debug("jbc It is not in primary panel state, skip esd check enable\n");
	     return;
	}
#endif
...
static void sde_connector_check_status_work(struct work_struct *work)
{
	...
	int rc = 0;
       ...
       if((lcd_states != DISPLAY_PRIMARY)||(lcd_states != DISPLAY_ONLY_PRIMARY)) {
            pr_debug("It is not in primary panel state, skip esd check\n");
	     return;
	}
//	   else{
  //              pr_err("It is primary panel,  esd check continue\n");
 //               }
	#endif
