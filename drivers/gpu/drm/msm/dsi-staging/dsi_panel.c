...
static int dsi_panel_change_switch(struct dsi_panel *panel,bool state)
{
	...
	struct dsi_switch_panel_config *s_config = &panel->switch_config;
	pr_debug("%s switch_panel_res=%d,state=%d\n",__func__,switch_panel_res,state);
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if((lcd_states == DISPLAY_POST_PRIMARY)||(lcd_states ==DISPLAY_POST_SECOND)){
		pr_debug("%s: it is switching do not need to setbacklight!\n", __func__);
		return rc;
		}
...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	//here is unnormal state
    if(!nubia_back_lcd_exist){
	     switch_panel_res = 0;
	...
	//here is normal state
		...
		if(switch_panel_res==1){
			...
			if(lcd_states != DISPLAY_POST_PRIMARY){
	      	...
	      	pr_debug("%s Kernel set lcd_states =%d \n",__func__,lcd_states);}
		}
		else{
			switch_panel_res=1;
	...
	pr_debug("%s Panel switching switch_panel_resource =%d lcd_states =%d \n",__func__,switch_panel_res,lcd_states);
	rc = nubia_dsi_panel_power_on(panel);
