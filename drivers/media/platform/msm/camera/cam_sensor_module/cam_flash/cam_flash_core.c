	...
	struct cam_flash_frame_setting *flash_data, enum camera_flash_opcode op)
{
	...
	if (op == CAMERA_SENSOR_FLASH_OP_FIRELOW) {
            ...
            if((flash_ctrl->torch_num_sources == 2) && (120 == flash_data->led_current_ma[0]) 
                ...
                flash_data->led_current_ma[0] = 200;
                flash_data->led_current_ma[1] = 0;
            }
            /*ZTEMT: fengxun add for FIRELOW current--------End*/
            if((flash_ctrl->torch_num_sources == 1) && (120 == flash_data->led_current_ma[0]) 
                && (0 == flash_data->led_current_ma[1])){
                CAM_ERR(CAM_FLASH, "changer to 10");
        ...
        cam_flash_off(flash_ctrl);//ZTEMT: jixd 20180913 add to force main flash
