...
static int tmd2725_offset_calibration(struct tmd2725_chip *chip)
{
	...
	** TMD2725_SHIFT_BINSRCH_TARGET  0x04 -->0x05 fix bug
	*/
...
static ssize_t tmd2725_offset_first_event_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct tmd2725_chip *chip = dev_get_drvdata(dev);
...
static ssize_t tmd2725_offset_first_event_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	int val;
	int rc;
	u8 offset_l,offset_h,calib_cfg;
	struct tmd2725_chip *chip = dev_get_drvdata(dev);
	u8 *sh = chip->shadow;
	rc = kstrtoint(buf, 0, &val);
	chip->pdata->offset_first_event = val;
	SENSOR_LOG_INFO("offset_first_event = %d\n", chip->pdata->offset_first_event);
	if(val==1)
	{
		ams_i2c_read(chip->client, TMD2725_REG_POFFSET_L, &offset_l);
		ams_i2c_read(chip->client, TMD2725_REG_POFFSET_H, &offset_h);
		SENSOR_LOG_INFO("offset_l:%d,offset_h:%d\n",offset_l,offset_h);
		chip->pdata->offset_h = offset_h;
		chip->pdata->offset_l = offset_l;
		offset_l =0;
		ams_i2c_write(chip->client, sh,TMD2725_REG_POFFSET_L, offset_l);
		ams_i2c_write(chip->client, sh,TMD2725_REG_POFFSET_H, offset_l);
		ams_i2c_read(chip->client, TMD2725_REG_CALIBCFG, &calib_cfg);
		chip->pdata->calib_cfg= calib_cfg;

		SENSOR_LOG_INFO("TMD3702_REG_CALIBCFG:0x%x\n",calib_cfg);
   		ams_i2c_modify(chip, sh, TMD2725_REG_CALIBCFG,
			TMD2725_MASK_PROX_AUTO_OFFSET_ADJUST,  0);

		ams_i2c_read(chip->client, TMD2725_REG_CALIBCFG, &calib_cfg);
		SENSOR_LOG_INFO("TMD2725_REG_CALIBCFG:0x%x\n",calib_cfg);
	}
	else if (val ==0)
	{
		SENSOR_LOG_INFO("restore offset_l:%d,offset_h:%d calib_cfg:0x%x\n",
            chip->pdata->offset_l ,chip->pdata->offset_h,chip->pdata->calib_cfg);
		ams_i2c_write(chip->client, sh,TMD2725_REG_POFFSET_L, chip->pdata->offset_l );
		ams_i2c_write(chip->client, sh,TMD2725_REG_POFFSET_H, chip->pdata->offset_h);
		ams_i2c_write(chip->client, sh,TMD2725_REG_CALIBCFG, chip->pdata->calib_cfg);
	}
	return size;
}
...
static ssize_t tmd2725_prox_thres_near_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct tmd2725_chip *chip = dev_get_drvdata(dev);
	return sprintf(buf, "%d\n", chip->params.prox_thres_near);
...
static struct device_attribute tmd2725_prox_attrs[] = {
	...
	__ATTR(prox_first_event, 0644, tmd2725_offset_first_event_show,  tmd2725_offset_first_event_store),
	__ATTR(prox_thres_near, 0440, tmd2725_prox_thres_near_show, NULL),
...
static int tmd2725_irq_handler_locked(struct tmd2725_chip *chip, u8 status)
{
	...
	if (status & TMD2725_ST_PRX_IRQ) {
	    ...
	    if (chip->prx_inf.prox_raw_debug) {
/* Promblem Number: PR000     Author:xuxiaohua,   Date:2018/7/14
   Description    : fix input report rel value 0 ,not update  */
            //psdata > 0 ? psdata : 1)
