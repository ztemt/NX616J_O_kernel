 ...
 *
 */
#define pr_fmt(fmt)	"HALL_DEVICE: %s: " fmt, __func__
...
#define SENSOR_LOG_ERROR(fmt, args...) pr_err("%d]"  fmt,__LINE__, ##args)
#define SENSOR_LOG_INFO(fmt, args...)  pr_info("%d]"  fmt,__LINE__, ##args)

#ifdef  DEBUG_ON
#define SENSOR_LOG_DEBUG(fmt, args...) pr_debug("%d] "  fmt,__LINE__, ##args)
#else
