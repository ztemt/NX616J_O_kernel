...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	if (IS_ERR_OR_NULL(bdata->pinctrl_info.pinctrl)) {
		pr_err("%s: failed to get pinctrl %ld\n", __func__, PTR_ERR(bdata->pinctrl_info.pinctrl));
		return PTR_ERR(bdata->pinctrl_info.pinctrl);
	}

	bdata->pinctrl_info.pin_active =  pinctrl_lookup_state(bdata->pinctrl_info.pinctrl,
		SYNAPTICS_PINCTRL_STATE_ACTIVE);

	if (IS_ERR_OR_NULL(bdata->pinctrl_info.pin_active)) {
		pr_err("%s:can not lookup %s pinstate %ld\n", __func__,
			SYNAPTICS_PINCTRL_STATE_ACTIVE, PTR_ERR(bdata->pinctrl_info.pin_active));
		return PTR_ERR(bdata->pinctrl_info.pin_active);
	}

	bdata->pinctrl_info.pin_suspend =  pinctrl_lookup_state(bdata->pinctrl_info.pinctrl,
		SYNAPTICS_PINCTRL_STATE_SUSPEND);

	if (IS_ERR_OR_NULL(bdata->pinctrl_info.pin_suspend)) {
		pr_err("%s:can not lookup %s pinstate %ld\n", __func__,
			SYNAPTICS_PINCTRL_STATE_SUSPEND, PTR_ERR(bdata->pinctrl_info.pin_suspend));
		return PTR_ERR(bdata->pinctrl_info.pin_suspend);
	}
	...
	retval = of_property_read_u32_array(np, "synaptics,full_raw_limit_upper", bdata->full_raw_limit_upper,
			TX_NUM_DEFAULT * RX_NUM_DEFAULT);
	if (retval < 0)
		pr_err("%s can not read synaptics,full_raw_limit_upper\n", __func__);

	retval = of_property_read_u32_array(np, "synaptics,full_raw_limit_lower", bdata->full_raw_limit_lower,
			TX_NUM_DEFAULT * RX_NUM_DEFAULT);
	if (retval < 0)
		pr_err("%s can not read synaptics,full_raw_limit_lower\n", __func__);
		...
		const struct i2c_device_id *dev_id)
{
	...
	dev_err(&client->dev, "%s:i2c_addr = %d\n", __func__, client->addr);
