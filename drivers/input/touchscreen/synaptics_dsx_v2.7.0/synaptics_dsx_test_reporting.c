		...
		struct device_attribute *attr, char *buf)
{
	...
	case F54_FULL_RAW_CAP_TDDI:
		...
		for (ii = 0; ii < tx_num; ii++) {
			for (jj = 0; jj < (rx_num - 1); jj++) {
				cnt = snprintf(buf, PAGE_SIZE - count, "%-4d ", *report_data_u16);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			g_tddi_noise_data_output[i*rx_num + j] = tddi_noise_max[i*rx_num + j] - tddi_noise_min[i * rx_num + j];
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct synaptics_dsx_board_data *board_data = f54->rmi4_data->hw_if->board_data;

	if (!g_tddi_full_raw_data_output)
		return -EINVAL;

	/*check the special code if failed to get report image output the error message*/
	if (g_flag_readrt_err) {
		kfree(g_tddi_full_raw_data_output);
	...
	for (i = 0, k = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			if ((*report_data_16) < board_data->full_raw_limit_lower[k] || (*report_data_16) > board_data->full_raw_limit_upper[k]) {
				pr_err("rawdata:%-4d tx_num:%d rx_num:%d  limit_lower:%-4d limit_upper:%-4d\n",
					*report_data_16, i, j, board_data->full_raw_limit_lower[k], board_data->full_raw_limit_upper[k]);
				fail_count++;
			}
			k++;
			report_data_16++;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0, offset = 0; i < tx_num * rx_num; i++) {
		tddi_rt95_part_one[i] = (signed short)(f54->report_data[offset]) | ((signed short)(f54->report_data[offset + 1]) << 8);
		offset += 2;
	}
	...
	for (i = 0, offset = buffer_size; i < tx_num * rx_num; i++) {
		tddi_rt95_part_two[i] = (signed short)(f54->report_data[offset]) | ((signed short)(f54->report_data[offset + 1]) << 8);
		offset += 2;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return test_sysfs_read_report_show(dev, attr, buf);
}
                ...
                struct device_attribute *attr, char *buf)
{
	...
	/*check_ito_test_flag will update in ***show func*/
	check_ito_test_flag = 0;

	test_sysfs_tddi_full_raw_store(dev, attr, type, sizeof(type));
	test_sysfs_tddi_full_raw_show(dev, attr, buf);

	test_sysfs_tddi_ee_short_store(dev, attr, type, sizeof(type));
	test_sysfs_tddi_ee_short_show(dev, attr, buf);

	/*check_ito_test_flag:=0 failed, >0 pass*/
	return sprintf(buf, "%s", check_ito_test_flag > 0 ? "0": "-1");
}
...
nubia_exit_attributes:
	sysfs_remove_group(f54->sysfs_dir, &nubia_attr_group);
