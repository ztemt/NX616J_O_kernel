...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	pr_err("[TP]%s: synaptics,power-on-state = %d \n", __func__, bdata->power_on_state);
		...
		bdata->busregen_gpio = of_get_named_gpio_flags(np,
				"synaptics,busregen-gpio", 0, NULL);
	if (bdata->busregen_gpio < 0){
		bdata->busregen_gpio = -1;
		pr_err("[TP]%s: can't get synaptics busregen_gpio-gpio !\n", __func__);
	}
	pr_err("[TP]%s: synaptics,busregen_gpio-gpio = %d \n", __func__, bdata->busregen_gpio);

		retval = of_property_read_u32(np, "synaptics,busregen-on-state",
				&value);
	if (retval < 0){
		bdata->busregen_on_state = -1;
		pr_err("[TP]%s: can't get synaptics busregen_on_state !\n", __func__);
	}
	else{
		bdata->busregen_on_state = value;
	}
	pr_err("[TP]%s: synaptics,busregen_on_state = %d \n", __func__, bdata->busregen_on_state);
