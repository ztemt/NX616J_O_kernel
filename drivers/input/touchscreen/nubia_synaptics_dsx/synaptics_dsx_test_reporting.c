...
#define PBUF_LEN 1024
        ...
        for (ii = 0; ii < tx_num - tx_vk; ii++) {
			...
			if(8 == ii && 0 == jj)
			{
				/*skip check this point, this point is cammera, no sensor*/
				continue;
				#if 0
				if(200 > report_data_16[ ii * rx_num + jj ]  || 460 < report_data_16[ ii * rx_num + jj ])
				{
				...
				#endif
			}else if( report_data_16[ ii * rx_num + jj ] < rmi4_data->hw_if->board_data->factory_low_cap || report_data_16[ ii * rx_num + jj ] > rmi4_data->hw_if->board_data->factory_up_cap) {
				dev_err(rmi4_data->pdev->dev.parent, "%s: test fail, ii=%d, jj=%d, val=%d\n", __func__, ii, jj, report_data_16[ ii * rx_num + jj ]);
				retval = -EINVAL;
				goto step1_done;
			}
