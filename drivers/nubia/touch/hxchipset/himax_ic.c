...
extern int g_i_new_FW_VER;
extern int g_i_new_CFG_VER;
extern int g_i_new_CID_MAJ; //GUEST ID
extern int g_i_new_CID_MIN; //VER for GUEST
extern unsigned char i_CTPM_FW_NEW[];
#endif
...
{
	if(!private_ts){
		E("%s private_ts is NULL exit!!\n",__func__);
		return;
	}
	I("%s: himax_sense_off start.\n",__func__);
...
bool himax_ic_package_check(struct i2c_client *client)
{
    ...
    //if i2c connect fail, stop and return
    if(!himax_enter_safe_mode(client))
    {
         ret_data = false;
         I("%s:I2c connect failed", __func__);
         return ret_data;
    }
    for (i = 0; i < 5; i++)
    {
...
#ifdef HX_FIX_NEW_PANEL
            g_i_new_FW_VER = i_CTPM_FW_NEW[FW_VER_MAJ_FLASH_ADDR]<<8 |i_CTPM_FW_NEW[FW_VER_MIN_FLASH_ADDR];
            g_i_new_CFG_VER = i_CTPM_FW_NEW[CFG_VER_MAJ_FLASH_ADDR]<<8 |i_CTPM_FW_NEW[CFG_VER_MIN_FLASH_ADDR];
            g_i_new_CID_MAJ = i_CTPM_FW_NEW[CID_VER_MAJ_FLASH_ADDR];
            g_i_new_CID_MIN = i_CTPM_FW_NEW[CID_VER_MIN_FLASH_ADDR];
#endif
#endif
