...
static int tfa9xxx_dbgfs_mtpex_set(void *data, u64 val)
{
	...
	mutex_lock(&tfa9xxx->dsp_lock);
	err = tfa2_dev_mtp_set(tfa9xxx->tfa, TFA_MTP_EX, val);
	mutex_unlock(&tfa9xxx->dsp_lock);
			       ...
			       struct snd_soc_dai *dai)
{
	...
	dev_dbg(&tfa9xxx->i2c->dev, "%s: state: %s (%d)\n", __func__, cmd ? "on":"off", cmd);

	switch (cmd) {
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		//dev_dbg(&tfa9xxx->i2c->dev, "clock on\n");
		break;

	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		//dev_dbg(&tfa9xxx->i2c->dev, "clock off\n");
		break;
	default:
		return -EINVAL;
	}
