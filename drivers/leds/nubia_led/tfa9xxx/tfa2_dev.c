...
#include "tfa9894_tfafieldnames.h" // TODO: CLKS
...
int tfa2_dev_start(struct tfa2_device *tfa, int next_profile, int vstep)
{
	...
	dev_dbg(&tfa->i2c->dev, " [SB] active profile:%s, next profile:%s\n",
...
{
//	TODO /* Make sure internal oscillator is not running for DSP devices (non-dsp and max1 this is no-op) */
...
////TODO name lookup ?
//static void show_bf(struct tfa2_device *tfa, nxpTfaBitfield_t *bf) {
//    uint16_t value;
//union {
//	uint16_t field;
//	nxpTfaBfEnum_t Enum;
...
//const char *name;
//
//name = tfaContBfName(bf->field, tfa->rev);
//
//value=bf->value;
...
{
	//dev_dbg(&tfa->i2c->dev, "write bitfield: 0x%04x=0x%04x\n", bitf->field, bitf->value); //TODO name lookup ?
	return tfa2_i2c_write_bf_volatile(tfa->i2c , bitf->field, bitf->value);
}
...
//	if ( rc < 0 )
	...
	if ( rc < 0 )
			return rc;
	/* now ACS can be set safely , keep in reset */
	rc = tfa2_i2c_write_cf_mem32_dsp_reset(tfa->i2c, 0x8100, &control, 1, TFA2_CF_MEM_IOMEM);
	if ( rc < 0 )
			return rc;
	rc = tfa2_i2c_read_bf(tfa->i2c, TFA9XXX_BF_ACS);
	if ( rc < 0 )
			return rc;
	if ( rc == 0 ) /* ACS should be1 */
	{
		dev_err(&tfa->i2c->dev, "ACS did not set!\n");
		rc = -EIO;
	}
	/* set I2CR  */
...
int tfa2_dev_set_state(struct tfa2_device *tfa, enum tfa_state state) {
	...
	dev_dbg(&tfa->i2c->dev, "set logical state: %s\n",
	...
	case TFA_STATE_POWERDOWN: /* PLL in powerdown, Algo up */
		...
		//TODO tfa2_i2c_write_bf(tfa->i2c ,TFA9XXX_BF_MANAOOSC, 1); /* 1 = off */
	...
	case TFA_STATE_POWERUP: /**< PLL to powerup, Algo can be up/warm */
		...
		rc = tfa9894_i2c_clock_stable_wait(tfa->i2c);
...
int tfa9894_i2c_clock_stable_wait(struct i2c_client *client)
{
	...
	do {
		ready = tfa2_i2c_read_bf(client,TFA9894_BF_CLKS);
		if  (ready == 1)
			break;
		else if (ready < 0)
			return -EIO;
...
static int get_mtp(struct tfa2_device *tfa, uint16_t addr, uint16_t *value)
{
	...
	rc = tfa9894_i2c_clock_stable_wait(i2c);
	if (rc < 0) {
		dev_err(&i2c->dev, "no clock\n");
		return rc;
	}

	rc = tfa2_i2c_mtp_read(i2c, addr);
...
int tfa2_dev_mtp_get(struct tfa2_device *tfa, enum tfa_mtp item)
{
	...
	rc = tfa9894_i2c_clock_stable_wait(tfa->i2c);
	if (rc < 0) {
		dev_err(&tfa->i2c->dev, "*****NO CLK when tfa2_dev_mtp_get*****\n");
	}
